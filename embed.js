(function () {
  ...
  function applyCalculatedMetrics(summary, commitments) {
    ...
    const totalKeyActions = Number(summary.totalKeyActions) || 0;
    summary.commitmentsTracked = commitments.length;
    summary.overallProgress = overallProgress;
    summary.completedKeyActions = totalKeyActions ? Math.round((overallProgress / 100) * totalKeyActions) : 0;
  }
  ...
  function styles() {
    return `
      .jw-widget { background:#fff; border:1px solid #dce3ec; border-radius:8px; box-shadow:0 14px 35px rgba(6,27,58,.12); color:#071831; cursor:pointer; font-family:Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; overflow:hidden; width:100%; }
  ...
  function render(data) {
    ...
    applyCalculatedMetrics(summary, commitments);
    ...
    mount.innerHTML = `
      <style>${styles()}</style>
      <section class="jw-widget" aria-label="${esc(summary.title)} metrics" role="link" tabindex="0">
        <div class="jw-top">
    ...
    const widget = mount.querySelector(".jw-widget");
