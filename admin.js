(function () {
  ...
  function recalculateMetrics() {
    ...
    const totalProgress = commitments.reduce((sum, item) => sum + clampPercent(item.progress), 0);
    const overallProgress = commitments.length ? Math.round(totalProgress / commitments.length) : 0;
    const totalKeyActions = Math.max(0, number(data.summary.totalKeyActions, 0));
    const completedKeyActions = totalKeyActions ? Math.round((overallProgress / 100) * totalKeyActions) : 0;
    const statusCounts = statuses.reduce((acc, status) => {
      acc[status] = commitments.filter(item => item.status === status).length;
      return acc;
    }, {});

    data.summary.commitmentsTracked = commitments.length;
    data.summary.overallProgress = overallProgress;
    data.summary.completedKeyActions = completedKeyActions;
    data.summary.statusBreakdown = statuses.map(status => ({
      status,
      percent: commitments.length ? Math.round((statusCounts[status] / commitments.length) * 100) : 0
    }));
  }
  ...
  function summaryFields() {
    ...
    const keyPercent = s.totalKeyActions ? Math.round((s.completedKeyActions / s.totalKeyActions) * 100) : 0;
    summaryForm.innerHTML = `
      <div class="calculated">
        <div><span>Overall progress</span><strong>${esc(s.overallProgress)}%</strong></div>
        <div><span>Key actions complete</span><strong>${esc(s.completedKeyActions)} / ${esc(s.totalKeyActions)} (${keyPercent}%)</strong></div>
      </div>
